...
for name,mod in unet.named_modules():
    if not (isinstance(mod,torch.nn.Conv2d) and mod.kernel_size==(3,3)): continue
    if name not in AMAX: continue
    C=mod.in_channels
    lam=None
    if MIG:
...
print(f"hooked {n_hook} convs, blockwise on {n_blk}",flush=True)
...
print("W4JSON:"+json.dumps({"tag":TAG,"act":ACT,"blk":BLK,"migrate":MIG,"wb":WB,"ab":AB,
 "hooked":n_hook,"blockwise":n_blk,"finite":bool(torch.isfinite(lat).all()),

# MoDiff overhead anatomy: warm-up, a_hat/o_hat, and an overlap hypothesis that failed

**Date** 2026-08-26 · **GPU** NVIDIA A40 (GA102, 84 SMs, 696 GB/s, 299 dense-int8 TOPS) · **Batch** 128

> **Revision 3.** Two independent audit rounds. Round 1 found **5 blockers, 10 significant, 10 minor**
> issues in revision 1; round 2 found that revision 2 had fixed the prose but **not the figures** (two
> blockers still shipped in plot titles), that its newly-written defence of §3's central negative
> result was **quantitatively wrong by ~3×**, and that three new claims contradicted the report's own
> tables. Corrections are marked **[R2]** and **[R3]**.
>
> The five round-1 blockers: o_hat's incremental byte count was +2 B not +4 B, breaking §2's headline
> framing; a "1.80× ceiling" was violated on 60% of calls; a byte-count inversion was circular and
> returned a value above the model's own maximum; a Spearman ρ = 0.00 computed by this report's own
> script was left in the JSON while only the post-deletion ρ = 1.00 was published; and "in every shape
> the baseline gains at least as much / the ratio never shrinks" was false and self-contradictory.
>
> **Findings 1–3 survive. §4's mechanism claim is downgraded to a correlation, and §3's is downgraded
> from "refuted" to "untested" for the mechanism while remaining refuted for the instrument.**
...
- **hand-computed** — §2's four per-shape bandwidth-efficiency figures and the ±30% gap; §4's 89.5%
  ...
  counts (a transcription of a `print()` in `warmup_cost_w1.py`).
...
| 2. What do a_hat/o_hat cost? | a_hat **14.8%**/**20.4%** of conv-block time (**6.4%**/**6.7%** of a step); o_hat **2.0–2.6%**. Per *byte*, a_hat is **≈2.4–3.0×**/**≈3.9–5.2×** dearer | high on the shares; the per-byte ratio is weighting-dependent |
| 3. Can a_hat be overlapped away? | **Not this way.** Batch-split pipelining leaves the ratio flat or worse; absolute overhead **rises 11.4% (3.46σ)**. But the schedule was ~70–92% in phase, so `conv ‖ GN` is **untested**, not refuted | aggregate resolved at 3.46σ; 3 of 5 shapes have ≥1 unresolved arm |
...
> is ≥91% arm-independent — the baseline gains 31% on the same shape. It is a **conv occupancy** finding
...
3. **The "negative controls" are median-centred by construction.** `warmup_cost_w1.py` subtracts
   ...
   a **3.87 ms spread** (±1.94 about the mean) = 0.024% of the sample. **[R3]** That is what calibrates the instrument.
...
*consistent* freq weighting: a_hat **6.4%** / **6.7%**, o_hat **1.1%** / **0.8%**. Revision 2 quoted
...
quote a range: **≈2.4–3.0× at W8A8** and **≈3.9–5.2× at W4A4**. **[R3]** The spread has two causes.
...
1.511, 1.511, 1.532) are the four lowest in the set and carry 55.9% of the GN time weight. **[R3]** **[R2]**
...
## 3. Attempt: overlap a_hat away — **the instrument is refuted, the mechanism is untested**
...
overhead *falls* 14.9%, and MoDiff's absolute saving exceeds the baseline's by 0.030 ms — but at
**2.56σ that difference is below this section's own 3σ bar**, so it is *consistent with* roughly 9%
of that shape's gain being MoDiff-specific overhead reduction (0.030 of MoDiff's own 0.333 ms) and
≥91% arm-independent, not established as such. **[R3]** A ratio metric with a shrinking denominator hid this;
...
`ev_offset` establishes an offset of one **GN** duration. Revision 2 put that at "8–12% of a stage"
and capped the achievable GN hiding at "12.2 pp" — **both wrong.** 12.2% is the *baseline* GN's share
of the *baseline* pair on the single shape `768,4x4`, which is the **minimum** of the set, and the
quantity a cap needs is the *MoDiff* GN's share. **[R3]**
...
So the streams are **~70–92% in phase**, not ~90%, and on `192,32x32` — 48% of the weighted time —
the intended overlap is exercised three times more than revision 2 admitted. More importantly,
perfect GN hiding could be worth up to **~35 pp**, which exceeds MoDiff's entire overhead on any
shape. **The validity failure could therefore change the verdict in principle**, and revision 2's
stated reason for saying it could not does not hold.

What actually carries the verdict is the aggregate, which revision 2 asserted without an
uncertainty: the freq-weighted absolute-overhead rise is **+2.003 ± 0.580 ms = 3.46σ** — resolved,
though only just, and by the same 3σ bar this section applies elsewhere. **[R3]**
...
work — and it is now the single most valuable follow-up in this report, because the mechanism is
...
| 17 | Cross-harness absolute ms/step (not just the ratio) | ✅ 5-shape totals agree to **+0.30%** (MoDiff) / **−1.04%** (baseline) **[R3]** |
| 18 | §3 tested its own mechanism | ❌ **no** — streams ~70–92% in phase, so `conv ‖ GN` was barely exercised **[R3]** |
...
1. **a_hat, at 14.8%/20.4% of conv-block time (6.4%/6.9% of a step), remains the largest
   ...
   it.** Note the narrower scope: §3's schedule was ~70–92% in phase, so inter-kernel concurrency as
   a *mechanism* is untested rather than refuted — closing it needs the phase-enforced rerun. **[R3]**

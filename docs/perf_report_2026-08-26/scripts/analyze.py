...
REMOVED 2026-08-26, at the project owner's request, after the route it tested was closed as
negative: section 3 (batch-split 2-stream pipelining), its two benchmark scripts
(bench_pipeline{,2}.py), its raw data (pipeline_result2.json) and every statistic derived from it.
That took with it the parts of section 4 that were sourced from the pipelining run -- the realized
`base gain` column, the SM-waste/gain rank correlation, and the e2e savings table. What section 4
keeps is trace-only: grids, blocks, waves, SM waste, and achieved compute/bandwidth. The verdict the
deleted section reached is recorded in one line in ../OPEN_ITEMS.md (C11).
"""
import csv
import json
import math
...
# =====================================================================
# 4. CONV OCCUPANCY -- wave quantization, from real nsys grids
# =====================================================================
...
# plot 4: what the conv leaves on the table -- occupancy waste, and what it achieves
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4.4))
srt = wdf.sort_values("sm_waste", ascending=False).reset_index(drop=True)
x = np.arange(len(srt))
ax1.bar(x, srt.sm_waste, bw * 1.6, color=C_GREY, zorder=3)
for i, v in enumerate(srt.sm_waste):
    ax1.text(i, v + 1.2, f"{v:.1f}%", ha="center", fontsize=8.5)
ax1.set_xticks(x)
ax1.set_xticklabels([f"{s}\n{int(b)} blk\n{w_:.2f} wave" for s, b, w_ in
                     zip(srt["shape"], srt.blocks, srt.waves)], fontsize=7.5, linespacing=1.5)
ax1.set_ylabel("idle SM-slots across all waves (%)")
ax1.set_ylim(0, 84)
ax1.set_title("wave quantization: SM slots the conv never fills", fontsize=10.5, loc="left")
despine(ax1)

ax2.bar(x - bw / 2, srt.pct_peak_compute, bw, color=C_BASE, label="% of peak int8 compute", zorder=3)
ax2.bar(x + bw / 2, srt.pct_peak_bw, bw, color=C_MOD, label="% of peak bandwidth", zorder=3)
for i, (a, b) in enumerate(zip(srt.pct_peak_compute, srt.pct_peak_bw)):
    ax2.text(i - bw / 2, a + 1.2, f"{a:.0f}", ha="center", fontsize=8)
    ax2.text(i + bw / 2, b + 1.2, f"{b:.0f}", ha="center", fontsize=8)
ax2.set_xticks(x)
ax2.set_xticklabels(srt["shape"], rotation=25, ha="right", fontsize=8.5)
ax2.set_ylabel("% of A40 peak")
ax2.set_title("neither roof is close: the machine is idle, not saturated", fontsize=10.5, loc="left")
...
print("\n=== 4. WAVE OCCUPANCY ===")
print(wdf[["shape", "grid", "blocks", "waves", "sm_waste",
           "pct_peak_bw", "pct_peak_compute"]].to_string(index=False))
print("\nwrote data/derived.json + 3 plots")
...
# (C) The decomposition ceiling on hiding a_hat, over the same 5 shapes section 4 uses. Pure
#     ablation arithmetic -- a_hat's share of (a_hat + o_hat), which IS MoDiff's conv-block overhead,
#     so this is an identity and not an achievable target. The second, pipeline-harness denominator
#     that used to give a "78-85%" range went with section 3.
a2["gn_hiding_cap"] = dict(
    cap_pp_freq_wtd=float(100 * (g5.a_hat_ms * g5.freq).sum() / (g5.base_total_ms * g5.freq).sum()),
    overhead_pct_freq_wtd=float(100 * ((g5.modiff_total_ms - g5.base_total_ms) * g5.freq).sum()
                                / (g5.base_total_ms * g5.freq).sum()))
a2["gn_hiding_cap"]["pct_of_overhead_removable"] = (
    100 * a2["gn_hiding_cap"]["cap_pp_freq_wtd"] / a2["gn_hiding_cap"]["overhead_pct_freq_wtd"])
a2["gn_hiding_cap"]["identity_residual"] = float(
    (g5.a_hat_ms + g5.o_hat_ms - (g5.modiff_total_ms - g5.base_total_ms)).abs().max())
...
# --- section 4 occupancy table, straight off the nsys grids ---
L = ["| shape | real grid | blocks | waves | last wave full | SM waste |",
     "|---|---|---|---|---|---|"]

...
**一个必须在 layer 级测量的技术原因（踩过）**：最初想在 op 级抓 shape，做法是给
`nn.Conv2d`/`nn.GroupNorm` 挂 `register_forward_hook`。结果 conv 只抓到 **19 次/step，而 profiler
实测是 84 次**——覆盖率仅 23%。原因是本项目的融合路径**绕过了 module.forward**：
`_prequant_gn_conv` 直接调 `conv.forward_from_int8(...)` 而不是 `conv(...)`，所以 hook 永不触发。
被融合掉的 conv 恰好是最重要的那些，于是 op 级 hook 会系统性漏掉主干、只剩边角。
`FusedResBlock`/`AttentionBlock` 是正常经 `forward` 调用的，所以 **layer 级 hook 不受此影响**——
这也是本节采用 layer 流水线而非 op 级测量的技术前提，而不仅是"更有意义"的偏好。
（那个 op 级抓取脚本已删除：它产出的 23% 覆盖率数据是错的，其文档字符串却声称
"coverage is complete by construction"，留着只会误导。）

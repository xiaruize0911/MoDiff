...
| **C20** **[new 2026-08-27]** | store a_hat at reduced SPATIAL RESOLUTION (1/f), upsampling on read -- the one framing that cuts a_hat's READ and WRITE together | a_hat read+write is 8 of the apply kernel's 10 sectors/warp (C17), so f=2 cuts a_hat traffic 4x: 10 -> 4 sectors, ~1.5x what the write-only 2.024 ms/step ceiling can remove | **MEASURED, REFUTED -- and it identifies the principle that closes the whole line.** Raised by the project owner after this line was called closed, correctly noting that every refutation on record attacked bytes-per-element (int8/fp8/companding) or number-of-writes (skip-K, deferred-write), never number-of-elements. The single-step screen on real tensors looked affordable: delta absmax inflates only **1.82x** at f=2 (0.86 bits) for -60% of the kernel's bytes. **The recursion kills it.** Replaying the real captured per-step target through both schemes, the invariant residual `max|a_hat - (a_hat_0 + sum dequant(codes))|` -- exactly what `conv(a_hat) - o_hat` sees -- stays bounded at **0.011** for production but grows **LINEARLY at ~+1.5/step for f=2, reaching 44.2 by step 30** against an O(1) activation; linear, not sqrt(t), so it is systematic bias, not noise. Under recursion the delta's inflation is **9.2x**, not 1.82x. Mechanism: `avg_pool -> upsample` loses each block's deviation from its mean, the delta re-encodes it, the accumulator keeps it, a_hat is re-coarsened and loses it again -- every step, same sign | **closed, negative** -> [ahat_downsample_2026-08-27](ahat_downsample_2026-08-27/FINDINGS.md). **The general principle, which predicts future attempts: a_hat is not a free-standing cache -- it is one half of an accumulator pair with o_hat, whose update only ever sees dequantized codes. ANY lossy transform of a_hat desynchronises the pair and the error accumulates instead of cancelling.** That covers int8/fp8 (refuted), companding (refuted), spatial downsampling (refuted here), and explains why deferred-write -- the ONLY lossless transform tried -- was the only one to reach the timing stage. Blocked in advance: adding `conv(e_t)` to o_hat would restore the invariant but costs a full extra conv per layer per step |

| **C21** **[new 2026-08-27]** | selective per-layer MoDiff, gated on the calibrated `step_gain_tail` -- the one a_hat-traffic lever C20's principle does NOT close, because it deletes a_hat rather than transforming it | **3.551 ms/step** on the five dominant shapes (W8A8), already sized by [ahat_overlap](ahat_overlap_2026-08-26/FINDINGS.md) conclusion 3, which explicitly noted it is **not bounded** by the write-only ceiling: dropping a layer removes its a_hat read, a_hat write AND o_hat together | **OPEN, and now has a criterion it previously lacked.** The int4 delta table generated 2026-08-27 (`integration/calibration/int4_delta_calibration.pt`, live-absmax, 20 steps/batch 4/6 rounds) gives per-layer `step_gain_tail`: **int4 median 0.71x, with 57/70 layers (81%) BELOW 1.0** and 94% below 2.0, against int8's median 12.45x and only 2/70 below 1.0. Per Theorem 4.3 a gain of 1 leaves the quantization error unchanged and below 1 it is strictly worse -- so at W4A4 MoDiff provably buys no precision on 81% of layers while paying a_hat read + a_hat write + o_hat on all of them. Today's measurement puts int4+MoDiff at 0.640 ms/sample-step against int4_baseline's 0.541 (18% overhead), so recovering most of that gap should land W4A4 near **1.47x vs fp16** instead of 1.28x, while keeping MoDiff on the ~13 layers where it actually reduces error | **open, and it is the recommended next step in this direction.** Needs: a per-layer gate on the calibrated gain, an FID run to confirm the dropped layers cost nothing, and a re-check of the 81% against a real `int4_delta_qdiff.pt` if one is ever produced (median 0.71 is a large enough signal that the calibration protocol is unlikely to explain it). Note this is a W4A4 lever specifically -- at int8 only 3% of layers qualify, consistent with W8A8+MoDiff being the shipped result |

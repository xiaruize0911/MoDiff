...
Run: python docs/bench_report_2026-08-16_gnfast/scripts/kernel_speedup.py    # no GPU
Writes docs/bench_report_2026-08-16_gnfast/KERNEL_SPEEDUP.md
"""
import collections
import json
import os
...
def main():
    ...
    o.append("")
    o.append("**T=16 is a regression, and a DELIBERATE one — correcting an earlier claim in this "
             "file.** Only 15 of its 25 calls fall back to `torch_sdpa_fp16` (49.4 µs); the other 10 run "
             "`flash_attn_int8_qi8packed_small_qout` at 65.8 µs, which is what puts the row below 1.00×. "
             "An earlier version of this paragraph called that a sign error. It is not: "
             "`quantized_std_attention.py:484` documents it, with its own measurements — *\"T16 is a "
             "KNOWN performance loss: the dp4a kernel costs ~T^2 while PyTorch's flash is launch-bound "
             "and flat … Taken deliberately for structural uniformity — one INT4 dataflow, no "
             "shape-dependent fallback to reason about. It also removes the separate "
             "`quant_attn_out_int4_pack` pass those blocks needed.\"* So the ~0.16 ms/step visible here "
             "is not free to reclaim: routing T=16 back to sdpa restores that separate pass, whose cost "
             "has to be counted against it. T=4 is the other way round and wins decisively (20.0 µs "
             "against sdpa's 49.2), which is why the same gate is right there.")
    o.append("")

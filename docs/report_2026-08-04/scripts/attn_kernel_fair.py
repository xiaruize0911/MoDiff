...
Q/K/V quantize. Cores:
  fp16 = GroupNorm + fp16 SDPA on PyTorch's DEFAULT backend (flash) -- the production path.
        A MATH-backend column is also reported, LABELLED, because the previous version of this
        script pinned MATH as the fp16 reference. That inflated every quantized speedup ~4x:
        MATH materializes the [BH,T,T] score matrix (token_major_attention.py:64-73 measures it at
        79.3 vs 8.9 ms/step against flash). The production forward pins no backend, so flash is
        what the quantized path must actually beat.
...
rows = []
tot = dict(fp16=0.0, int8=0.0, int4=0.0, gn=0.0, fp16_math=0.0)
...
for (C, nh, hd, T, Hs, cnt) in SHAPES:
    ...
    # Production fp16 reference: no pinned backend, exactly as token_major_attention._SDPA_CTX
    # leaves it. This is the number every quantized ratio below is divided by.
    t_a16 = bench(lambda: F.scaled_dot_product_attention(q4v, k4v, v4v, scale=sc))
    # MATH kept only as a labelled control, to show how much the old reference inflated things.
    with sdpa_kernel(SDPBackend.MATH):
        t_a16_math = bench(lambda: F.scaled_dot_product_attention(q4v, k4v, v4v, scale=sc))
    ref = None
    with torch.no_grad():
    ...
    rows.append(dict(C=C, hd=hd, T=T, count=cnt, eligible=int(eligible), gn_us=round(t_gn, 1),
                     ...
                     attn16_us=round(t_a16, 1), attn16_math_us=round(t_a16_math, 1),
                     math_inflation=round(t_a16_math / t_a16, 2), attn8_us=round(t_a8, 1) if t_a8 else "", attn4_us=round(t_a4, 1) if t_a4 else "",
    ...
    tot["fp16_math"] += cnt * (t_gn + t_a16_math)
...
print(f"  CONTROL: had the fp16 reference been pinned to the MATH backend (as the 07-23 version of\n"
      f"  this script did) the int8 ratio would read "
      f"{tot['fp16_math']/tot['int8']:.2f}x instead of {tot['fp16']/tot['int8']:.2f}x -- "
      f"an inflation of {tot['fp16_math']/tot['fp16']:.2f}x from the reference alone.")
rows.append(dict(C="WEIGHTED_TOTAL", hd="", T="", count=21, eligible="", gn_us=round(tot["gn"], 1),
                 q8_us="", q4_us="", attn16_us="", attn16_math_us="", math_inflation="",
                 attn8_us="", attn4_us="",

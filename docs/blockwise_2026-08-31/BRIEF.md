...
### speedup 随 (B, N, H, W) 的变化，及它为什么长这样

speedup 就是上面两条时间曲线的比值，形状不是一个独立的新事实——是把已经拟合出的幂律直接代数运算一下。
[`scripts/kernel_axis_sweep.py`](scripts/kernel_axis_sweep.py) 两者都算了，图直接画的是比值。

![speedup 逐轴扫描](plots/fig11_speedup_axis_sweep.png)

**B — batch**

| B | fp16/base (GN) | fp16/MoDiff (GN) | fp16/base (conv) | fp16/MoDiff (conv) | **fp16/base (总)** | **fp16/MoDiff (总)** | **base/MoDiff (总)** |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 8 | 0.83× | 0.84× | 1.79× | 1.81× | **1.42×** | **1.43×** | **1.01×** |
| 16 | 2.25× | 1.12× | 1.34× | 1.45× | **1.50×** | **1.35×** | **0.90×** |
| 32 | 2.15× | 1.16× | 1.47× | 1.37× | **1.61×** | **1.30×** | **0.81×** |
| 64 | 2.49× | 1.36× | 1.49× | 1.54× | **1.70×** | **1.48×** | **0.87×** |
| 128 | 2.56× | 1.38× | 1.57× | 1.63× | **1.77×** | **1.55×** | **0.87×** |
| 256 | 2.58× | 1.40× | 1.63× | 1.70× | **1.82×** | **1.60×** | **0.88×** |

**N — 通道数 (Cin=Cout)**

| N | fp16/base (GN) | fp16/MoDiff (GN) | fp16/base (conv) | fp16/MoDiff (conv) | **fp16/base (总)** | **fp16/MoDiff (总)** | **base/MoDiff (总)** |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 128 | 3.34× | 2.47× | 1.31× | 1.31× | **2.17×** | **1.89×** | **0.87×** |
| 192 | 3.18× | 1.96× | 0.92× | 0.95× | **1.48×** | **1.31×** | **0.89×** |
| 256 | 2.86× | 1.51× | 1.43× | 1.51× | **1.79×** | **1.51×** | **0.84×** |
| 384 | 2.55× | 1.38× | 1.56× | 1.63× | **1.77×** | **1.55×** | **0.88×** |
| 512 | 2.14× | 1.23× | 1.72× | 1.77× | **1.79×** | **1.62×** | **0.91×** |
| 768 | 1.99× | 1.10× | 1.85× | 1.91× | **1.87×** | **1.74×** | **0.93×** |
| 1152 | 1.89× | 1.05× | 1.90× | 1.92× | **1.89×** | **1.79×** | **0.94×** |
| 1536 | 1.62× | 0.99× | 1.93× | 1.93× | **1.91×** | **1.82×** | **0.95×** |

**H — 高**

| H | fp16/base (GN) | fp16/MoDiff (GN) | fp16/base (conv) | fp16/MoDiff (conv) | **fp16/base (总)** | **fp16/MoDiff (总)** | **base/MoDiff (总)** |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 2 | 3.26× | 1.41× | 1.36× | 1.45× | **1.69×** | **1.44×** | **0.85×** |
| 4 | 4.92× | 2.40× | 1.37× | 1.42× | **2.06×** | **1.75×** | **0.85×** |
| 8 | 3.58× | 1.90× | 1.48× | 1.53× | **1.91×** | **1.65×** | **0.86×** |
| 16 | 2.56× | 1.39× | 1.56× | 1.63× | **1.76×** | **1.55×** | **0.88×** |
| 32 | 2.03× | 1.13× | 1.71× | 1.79× | **1.77×** | **1.57×** | **0.89×** |

**W — 宽**

| W | fp16/base (GN) | fp16/MoDiff (GN) | fp16/base (conv) | fp16/MoDiff (conv) | **fp16/base (总)** | **fp16/MoDiff (总)** | **base/MoDiff (总)** |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 2 | 3.15× | 1.35× | 1.32× | 1.40× | **1.64×** | **1.38×** | **0.84×** |
| 4 | 5.41× | 2.37× | 1.50× | 1.38× | **2.27×** | **1.71×** | **0.75×** |
| 8 | 3.68× | 1.94× | 1.48× | 1.53× | **1.93×** | **1.67×** | **0.86×** |
| 16 | 2.55× | 1.38× | 1.54× | 1.60× | **1.74×** | **1.53×** | **0.88×** |
| 32 | 2.06× | 1.16× | 1.65× | 1.72× | **1.73×** | **1.54×** | **0.89×** |

**数学上：** 如果两个 kernel 对同一个变量都是幂律，$t_A(x) = C_A x^{p_A}$、$t_B(x) = C_B x^{p_B}$，
它们的比值**也是**幂律：

$$\text{speedup}(x) = \frac{t_A(x)}{t_B(x)} = \frac{C_A}{C_B} \, x^{\,p_A - p_B}$$

这本身在 log-log 图上也是直线，斜率就是**两个指数的差**。两个推论，表里都能看到：

1. **指数相等时 speedup 是平的。** B、H、W 这三个轴上，上一轮量出来每个 kernel 的指数都接近 1
   （GN 是 0.86–0.96，conv 是 0.83–0.99，两个 arm 都一样），两边都是同一次数的算力/带宽瓶颈，
   $p_A - p_B$ 差很小，speedup 几乎不动：fp16/MoDiff 总路径在整个 B 扫描范围内只有 1.30×–1.82×，
   H 上 1.38×–1.76×，W 上 1.38×–1.71×。
2. **指数不等时 speedup 自己变成一条有斜率的幂律。** N 轴上的 conv 就是这样：fp16 conv 的指数测出来
   约 1.9–1.95（`Cin × Cout` 都在长大，理想值是 2），int8 conv 只有 1.66–1.68，差了约 0.25–0.3。
   这个差**就是** fp16/int8 conv-speedup 那条线自己的 log-log 斜率——它从 N=128 的
   1.31× 涨到 N=1536 的 1.93×，因为 int8 conv 在这个 tile 尺寸范围内
   始终没有追平 fp16 的算力效率。

**为什么总路径的 speedup 比任何单个部件都更平。** 总时间是**和**（$t_{gn}(x) + t_{conv}(x)$），
不是单一幂律，两个和的比值不是幂律——它在两个 x 时刻各自占主导的项之间插值：

$$\text{speedup}_{\text{总}}(x) = \frac{t_{gn}^{fp16}(x) + t_{conv}^{fp16}(x)}{t_{gn}^{arm}(x) + t_{conv}^{arm}(x)}$$

在 N=128 时，对 fp16 来说 GN 的绝对时间还比 conv 大（0.202 ms vs conv 的
0.108 ms），所以总 speedup 更贴近 GN 的比值（3.34×）。到了
N=1536，conv 反超一个数量级以上（11.59 ms vs GN 的 0.788 ms），
总 speedup 就收敛到 conv 的比值上（总数 1.91× 对上纯 conv 的
1.93×）。这个交叉过程——不是干净的幂律，而是权重随 $x$ 移动的加权混合——
就是为什么图下排的总量曲线明显比上排任何一条单独的 GN 或 conv 曲线更平滑、更不极端。

表里还有一点理想模型没完全预测到：GN-only 的 speedup 在 B/H/W 上也不是完全平的（fp16/baseline GN
先涨后跌，在 H=W=4 附近有个峰；fp16/MoDiff GN 随 N 单调下降）。两个 GN kernel 在实践中指数并不
**完全**相等——fp16 的普通 GroupNorm 和 baseline 的 fast-reduce kernel 在不同形状下各自的 occupancy
甜点不同，这是"指数完全相等"这个理想故事的二阶偏差，表现为曲线的弯曲而不是一条绝对水平的直线。

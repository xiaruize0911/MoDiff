...
### 全部 6 个生产/参照 kernel 随 (B, N, H, W) 的变化

同一套轴、同一个默认点，但现在覆盖每条生产路径的两个 kernel（GN 融合 + conv），不只是 conv。
[`scripts/kernel_axis_sweep.py`](scripts/kernel_axis_sweep.py)。

![kernel 逐轴扫描](plots/fig10_kernel_axis_sweep.png)

**B — batch**（N=384, H=16, W=16）

| B | fp16 GN | base GN(fast) | MoDiff GN | fp16 conv | base conv(D1) | MoDiff conv(D2) | **fp16 总** | **base 总** | **MoDiff 总** |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
...
| N | fp16 GN | base GN(fast) | MoDiff GN | fp16 conv | base conv(D1) | MoDiff conv(D2) | **fp16 总** | **base 总** | **MoDiff 总** |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
...
| H | fp16 GN | base GN(fast) | MoDiff GN | fp16 conv | base conv(D1) | MoDiff conv(D2) | **fp16 总** | **base 总** | **MoDiff 总** |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
...
| W | fp16 GN | base GN(fast) | MoDiff GN | fp16 conv | base conv(D1) | MoDiff conv(D2) | **fp16 总** | **base 总** | **MoDiff 总** |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
...
1. **三条 conv 曲线（fp16、D1、D2）几乎全程重合。** 和融合对那张表里的"conv 打平"是同一个结论，
   这里在每个轴上都是三条几乎叠在一起的线，不是单一比值——没有哪个形状 MoDiff 的 conv 会明显偏离 baseline。
2. **GN 阶段的排序（baseline fast < MoDiff < fp16）在每个轴、每个取值上都稳定**，不只是默认点。
   baseline 的 GN+quant kernel 在所有 B/N/H/W 上都是三者里最便宜的；MoDiff 的 GN+delta 居中；
   fp16 的普通 GN 在 B≥32 之后最贵（两个 int8 GN kernel 写的都比 fp16 的 `normed` 少，MoDiff 尽管
   还要多读写一次 `a_hat`）。
3. **这个排序在最小的 batch 上会反过来。** 在 B=8 时，fp16 GN（0.021 ms）已经比
   MoDiff（0.025 ms）**便宜**，也接近 baseline（0.026 ms）——
   这个尺度上三者都是 launch-bound，融合对那节的字节数论证还没生效。MoDiff 真正开始付出 GN 税
   大概从 B=32–64 开始。
4. **N=192 在这里也冒出来了**，跟 `axis_sweep.py` 那个发现是独立复现的：baseline 的 conv 从
   N=128 到 N=192 直接从 0.073 跳到 0.223 ms（几乎不比 N=384 的按比例份额少），和之前 fp16-vs-int8
   凹陷是同一个半空 tile 信号。

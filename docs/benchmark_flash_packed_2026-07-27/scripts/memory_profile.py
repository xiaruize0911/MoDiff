"""Peak-memory profile across all 5 modes, same session, same methodology as
final_speedup_and_breakdown.py (isolated per-mode model construction, GPU clock
burn-in shared once). Reports torch.cuda.max_memory_allocated (tensors only) and
max_memory_reserved (allocator's actual VRAM footprint, including fragmentation/
caching overhead) at batch=128, alongside the already-known ms/step for context.
"""
...
BATCH = 128
WARMUP, TIMED, RUNS = 10, 30, 3  # lighter than the speed benchmark -- this is about memory, not timing precision

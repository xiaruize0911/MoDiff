...
static_quantize_and_update_ahat_kernel_int8_half_cache[_silu]).
...
num_channels%2==0 gate) and with/without SiLU.
"""
...
def run_case(N, C, H, W, use_smooth, use_silu, seed):
    ...
    fn = mc.step1_static_quantize_fprop_silu if use_silu else mc.step1_static_quantize_fprop
    trace = []
    ...
    for shape in SHAPES:
        for use_smooth in (False, True):
            for use_silu in (False, True):
                yield shape["N"], shape["C"], shape["H"], shape["W"], use_smooth, use_silu
...
def main():
    ...
    if a.capture or not os.path.exists(CAPTURE_PATH):
        results = {}
        for i, (N, C, H, W, use_smooth, use_silu) in enumerate(all_cases()):
            key = f"N{N}_C{C}_smooth{int(use_smooth)}_silu{int(use_silu)}"
            results[key] = run_case(N, C, H, W, use_smooth, use_silu, seed=i)
    ...
    for i, (N, C, H, W, use_smooth, use_silu) in enumerate(all_cases()):
        key = f"N{N}_C{C}_smooth{int(use_smooth)}_silu{int(use_silu)}"
        trace = run_case(N, C, H, W, use_smooth, use_silu, seed=i)
        ref_trace = ref[key]
        per_iter_ok = [torch.equal(o, ro) and torch.equal(ah, rah)
                       for (o, ah), (ro, rah) in zip(trace, ref_trace)]
        ok = all(per_iter_ok)
        all_ok &= ok
        print(f"[{'PASS' if ok else 'FAIL'}] {key:28s} per_iter_equal={per_iter_ok}")

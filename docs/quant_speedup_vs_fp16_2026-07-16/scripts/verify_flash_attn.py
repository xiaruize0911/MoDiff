"""Verify + benchmark the flash attention wiring (§8 kernels) on the int8 churches UNet.
Baseline = fp16 SDPA (flash OFF). Measures: (1) quality — UNet-forward rel-L2 of flash
vs baseline, against the model's own nondeterminism floor; (2) latency per-step, OFF vs ON.
Toggles TokenMajorAttentionBlock._flash post-setup. Batch via E2E_BATCH (default 64)."""
...
print(f"flash-eligible attention blocks: {set_score(model, True)}")
...
# --- quality: baseline (fp16 SDPA) vs flash (after self-calib warm) ---
...
print(f"  OFF vs flash (attn quant err): rel-L2={rel(a1, b1):.3e}")
...
print(f"  flash OFF (fp16 SDPA): min={off_min:.3f}  median={off_med:.3f} ms/step")
print(f"  flash ON             : min={on_min:.3f}  median={on_med:.3f} ms/step")

"""E2E per-component timing profile across the 5 modes, CURRENT (fusion-fixed) state, b128.

Same as benchmark_5mode/e2e_timing_profile.py (torch.profiler CUDA self-time bucketed by cat(),
ms/step; independent wall; gpu_busy = sum buckets) so it is directly comparable to
benchmark_5mode_2026-07-21/data/e2e_timing_profile.csv. cat() adds accumulate_from_half (the Phase-2
deep-fuse cache-accumulate kernel) to the modiff-cache bucket. Writes data/e2e_timing_profile.csv.
"""
...
rows = []
print(f"E2E per-component timing profile @ b{BATCH} (ms/step) -- CURRENT fusion-fixed state\n")
for (label, mode) in VERS:
    agg, wall = run(mode)
    gpu = sum(agg.values())
    rows.append(dict(mode=label, **{b: round(agg[b], 3) for b in BUCKETS}, gpu_busy=round(gpu, 3), wall=round(wall, 3)))

...
#define KERNEL_LAUNCH_CODE                                                       \
  ...
  auto kernel_func =                                                             \
      w8a8ofp16_kernel<CTA_M, CTA_N, CTA_K, WARP_M, WARP_N, WARP_K, STAGES>;        \
...
template <int CTA_M, int CTA_N, int CTA_K, int WARP_M, int WARP_N, int WARP_K,
          int STAGES>
__global__ void w8a8ofp16_kernel(int8_t *__restrict__ A, int8_t *__restrict__ B,
...
torch::Tensor w8a8_of16_nobias_weight_sym_qserve(torch::Tensor input,
                                torch::Tensor weight,
                                torch::Tensor scale_input,
                                torch::Tensor scale_weight)
{
  // input: [num_in_feats, num_in_channels]
  // weight: [num_out_channels, num_in_channels]
  // scale_weight: [num_out_channels]
  // scale_input: [num_in_feats]
  ...
  CHECK_DTYPE(scale_weight, torch::kFloat16);
  CHECK_DTYPE(scale_input, torch::kFloat16);

  int num_in_feats = input.size(0);
  int num_in_channels = input.size(1);
  auto in_feats = reinterpret_cast<int8_t *>(input.data_ptr<int8_t>());
  auto kernel = reinterpret_cast<int8_t *>(weight.data_ptr<int8_t>());
  auto wscales = reinterpret_cast<half2 *>(scale_weight.data_ptr());
  auto ascales = reinterpret_cast<half *>(scale_input.data_ptr());
  auto options =
      torch::TensorOptions().dtype(torch::kFloat16).device(input.device());
  at::Tensor _out_feats =
      torch::empty({num_in_feats, weight.size(0)}, options);
  int num_out_feats = _out_feats.size(-2);
  int num_out_channels = _out_feats.size(-1);

  CHECK_SHAPE(weight, num_out_channels, num_in_channels);
  CHECK_SHAPE(scale_weight, num_out_channels);
  CHECK_SHAPE(scale_input, num_in_feats);

  auto out_feats = reinterpret_cast<half *>(_out_feats.data_ptr<at::Half>());

  if (num_in_feats % 128 || num_in_channels % 64 || num_out_channels % 128) {
    std::ostringstream err_msg;
    err_msg << "Input shapes must be multiples of 128, 64, and 128 respectively, "
            << "but got: " << num_in_feats << ", " << num_in_channels << ", "
            << num_out_channels;
    throw std::invalid_argument(err_msg.str());
  }

  if (num_out_feats > 128 && num_out_channels % 256 == 0)
  {

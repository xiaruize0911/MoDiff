...
from qdiff.quant_layer_int4 import QuantModuleINT4, UniformAffineQuantizerINT4
from qdiff.quant_block_int4 import BaseQuantBlockINT4
from qdiff.quant_model_int4 import QuantModelINT4
from qdiff.adaptive_rounding import AdaRoundQuantizer
from qdiff.quant_layer_int4 import UniformAffineQuantizerINT4

logger = logging.getLogger(__name__)


def save_inp_oup_data_int4(model: QuantModelINT4, layer: Union[QuantModuleINT4, BaseQuantBlockINT4], cali_data: torch.Tensor,
    ...
    :param model: QuantModelINT4
    :param layer: QuantModuleINT4 or QuantBlockINT4
    ...
    get_inp_out = GetLayerInpOutINT4(model, layer, device=device, asym=asym, act_quant=act_quant)
    cached_batches = []
...
def save_inp_oup_data_modiff_int4(model: QuantModelINT4, layer: Union[QuantModuleINT4, BaseQuantBlockINT4], cali_data: torch.Tensor,
                      asym: bool = False, act_quant: bool = False, batch_size: int = 32, keep_gpu: bool = True,
                      cond: bool = False, is_sm: bool = False):
    """
    Save input data and output data of a particular layer/block over calibration dataset.
...
def save_grad_data_int4(model: QuantModelINT4, layer: Union[QuantModuleINT4, BaseQuantBlockINT4], cali_data: torch.Tensor,
    ...
    """
    device = next(model.parameters()).device
    get_grad = GetLayerGradINT4(model, layer, device, act_quant=act_quant)
    cached_batches = []
    torch.cuda.empty_cache()
...
class GetLayerInpOutINT4:
    def __init__(self, model: QuantModelINT4, layer: Union[QuantModuleINT4, BaseQuantBlockINT4],
    ...
    def __call__(self, model_input):
        ...
        with torch.enable_grad():
            try:
                ...
                quantize_model_till_int4(self.model, self.layer, self.act_quant)
...
def quantize_model_till_int4(model: QuantModuleINT4, layer: Union[QuantModuleINT4, BaseQuantBlockINT4], act_quant: bool = False):
    """
    ...
    for name, module in model.named_modules():
        if isinstance(module, (QuantModuleINT4, BaseQuantBlockINT4)):
...
def convert_adaround_int4(model):
    for name, module in model.named_children():
        if isinstance(module, QuantModuleINT4):
        ...
        elif isinstance(module, BaseQuantBlockINT4):
            if module.ignore_reconstruction is True:
                ...
                for name, sub_module in module.named_modules():
                    if isinstance(sub_module, QuantModuleINT4):
            ...
            convert_adaround_int4(module)


def resume_cali_model_int4(qnn, ckpt_path, cali_data, quant_act=False, act_quant_mode='qdiff', cond=False):
    ...
    if quant_act:       
        ...
        for m in qnn.model.modules():
            if isinstance(m, AdaRoundQuantizer):
                m.zero_point = nn.Parameter(m.zero_point)
                m.delta = nn.Parameter(m.delta)
            elif isinstance(m, UniformAffineQuantizerINT4):

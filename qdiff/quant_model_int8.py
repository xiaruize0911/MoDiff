"""
INT8 Quantized Model Wrapper for MoDiff

This module provides a complete INT8 quantized model wrapper that follows
the MoDiff paper's methodology with native TensorRT INT8 support.

Key Features:
1. Full model INT8 quantization with modulation support
2. MoDiff error compensation mechanism
3. Native TensorRT INT8 backend support
...
class QuantModelINT8(nn.Module):
    ...
    Args:
        ...
        trt_engine_path: Optional TensorRT engine for INT8 inference
    ...
    Example:
        ...
        >>> weight_params = {'n_bits': 8, 'symmetric': True, 'scale_method': 'mse'}
        >>> act_params = {'n_bits': 8, 'symmetric': True, 'scale_method': 'mse'}
        >>> 
        >>> qmodel = QuantModelINT8(model, weight_params, act_params, modulate=True)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Default INT8 quantization params
        ...
        # Convert model layers to INT8 quantized modules
        ...
        logger.info(
            f"[QuantModelINT8] Initialized with {self._count_quant_modules()} INT8 layers, "
    ...
    def _quant_module_refactor(
        ...
        """Replace Conv2d/Linear with QuantModuleINT8."""
    ...
    def _init_trt_backend(self, engine_path: str) -> None:
        """Initialize TensorRT backend for INT8 inference."""
        try:
            from trt.int8_inference import TRTInt8Wrapper
            
            engine_path = Path(engine_path).resolve()
            if engine_path.exists():
                self.trt_wrapper = TRTInt8Wrapper(str(engine_path))
                self.use_trt_backend = True
                logger.info(f"[QuantModelINT8] Loaded TensorRT INT8 engine from {engine_path}")
            else:
                logger.warning(f"[QuantModelINT8] TensorRT engine not found: {engine_path}")
        except ImportError:
            logger.warning("[QuantModelINT8] TensorRT not available")
    ...
    def save_quantized_model(self, path: str) -> None:
        ...
        logger.info(f"[QuantModelINT8] Saved quantized model to {path}")
    ...
    def load_quantized_model(
        ...
        logger.info(f"[QuantModelINT8] Loaded quantized model from {path}")
    ...
    def export_scales_for_trt(self, output_path: str) -> None:
        ...
        # Save as JSON for TensorRT (dynamic ranges)
        import json
        trt_scales = {}
        for layer_name, layer_scales in scales.items():
            if layer_scales.get('act_dynamic_range') is not None:
                trt_scales[layer_name] = {
                    'dynamic_range': float(layer_scales['act_dynamic_range']),
                    'scale': float(layer_scales['act_scale']) if layer_scales['act_scale'] is not None else None,
                    'n_bits': 8,
                }
                
        json_path = output_path.with_suffix('.json')
        with open(json_path, 'w') as f:
            json.dump(trt_scales, f, indent=2)
        logger.info(f"[QuantModelINT8] Exported {len(trt_scales)} dynamic ranges to {json_path}")
        ...
        logger.info(f"[QuantModelINT8] Exported {len(flat_scales)} scales to {npz_path}")


def recon_model_int8(model: QuantModelINT8) -> None:
    """
    Reconstruct INT8 quantized model using MoDiff methodology.
    
    This function calibrates INT8 scales using MSE-based search,
    following the paper's approach for optimal quantization.
    """
    logger.info("[recon_model_int8] Starting INT8 model reconstruction...")
    ...
    logger.info("[recon_model_int8] INT8 reconstruction complete")


def recon_model_modiff_int8(model: QuantModelINT8) -> None:
    """
    Reconstruct INT8 quantized model with MoDiff modulation.
    
    This enables the paper's key innovation: quantizing residuals
    instead of absolute values for reduced error.
    """
    logger.info("[recon_model_modiff_int8] Starting INT8 MoDiff reconstruction...")
    ...
    logger.info("[recon_model_modiff_int8] INT8 MoDiff reconstruction complete")

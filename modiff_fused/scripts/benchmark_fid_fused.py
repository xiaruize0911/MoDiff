...
# Add paths
sys.path.insert(0, str(Path(__file__).parent.parent.parent)) # MoDiff root
sys.path.insert(0, str(Path(__file__).parent.parent)) # modiff_fused
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "modiff_triton")) # for ddim/ldm if needed
...
# New Fused Kernels
import sys
sys.path.append("/workspace/MoDiff/modiff_fused")
from fused_kernels.fused_modiff import fused_modiff_gemm
from fused_kernels.fused_conv import fused_modiff_conv2d
    ...
    def forward(self, x, use_modiff=True):
        ...
        if self.act_scale is not None and self.has_act_scale:
            if use_modiff and self.prev_quantized is not None:
                if self.act_bit == 8:
                    # For Linear layers, the fused kernel handles quantization and cache update internally
                    if isinstance(self.original_layer, nn.Linear):
                        # Skip explicit quantization here, pass raw X and X_prev to kernel
                        x_int8 = None # Signal to use fused path
                        x_fp = x # Placeholder
                    else:
                        # For Conv layers, we still need to update cache explicitly (for now)
                        # MoDiff modulated quantization returns INT8 residual and updated cache
                        x_int8, _, act_scale = modulated_quantize_int8(
                            x, self.prev_quantized, scale=None, out_a_hat=self.prev_quantized
                        )
                        x_fp = self.prev_quantized
        ...
        if (
            ...
            if use_modiff and self.prev_output is not None:
                # Fused Path (Quant + GEMM + Accum + CacheUpdate)
                # x is FP32/16, prev_quantized is FP32/16 (cache)
                orig_shape = x.shape
                K = orig_shape[-1]
                x_2d = x.view(-1, K).contiguous()
                prev_q_2d = self.prev_quantized.view(-1, K).contiguous()
                prev_out_2d = self.prev_output.view(-1, weight_int8.shape[1])
                
                out_2d = fused_modiff_gemm(
                    x_2d, prev_q_2d, weight_int8, prev_out_2d, self.weight_scale
                )
                out = out_2d.view(*orig_shape[:-1], weight_int8.shape[1])
                
                # Note: prev_q_2d is updated in-place by the kernel!
                # But we need to ensure the view update reflects in self.prev_quantized?
                # PyTorch views share memory, so yes.
            elif x_int8 is not None:
                # Standard path (first step or no modulation)
                ...
                # Fallback if x_int8 is None (should not happen for first step if calibrated)
                # But if we skipped quantization above for Linear, we need to handle first step here
                if self.act_scale is not None:
                     x_int8, act_scale = quantize_symmetric_int8(x, self.act_scale)
                     # Update cache
                     if self.prev_quantized is None or self.prev_quantized.shape != x.shape:
                         self.prev_quantized = dequantize_int8(x_int8, act_scale)
                     else:
                         self.prev_quantized.copy_(dequantize_int8(x_int8, act_scale))
                     ...
                     return nn.functional.linear(x, weight_fp, self.original_layer.bias)
        ...
        if (
            ...
            self.weight_bit == 8 and self.act_bit == 8 and
            x_int8 is not None and act_scale is not None and torch.cuda.is_available() and
            self.original_layer.in_channels >= 16  # Skip first layer (C=3) to avoid OOB pointers in Triton
        ):
            ...
            if (
                ...
                if use_modiff and self.prev_output is not None:
                    # Fused path (GEMM + Accumulate)
                    out = conv2d_int8_triton_accumulate(
                        x_int8, self.quantized_weight, self.prev_output,
                        act_scale, self.weight_scale, bias=None
                    )
                else:

...
from kernels.gemm_w8a8 import gemm_w8a8 as triton_gemm_w8a8, gemm_w8a8_accum
    ...
    def forward(self, x, use_modiff=True):
        ...
        if self.act_scale is not None and self.has_act_scale:
            if use_modiff and self.prev_quantized is not None:
                if self.act_bit == 8:
                    ...
                    # In-place update: pass prev_quantized as out_a_hat
                    x_int8, _, act_scale = modulated_quantize_int8(
                        x, self.prev_quantized, scale=None, out_a_hat=self.prev_quantized
                    )
                    # self.prev_quantized is already updated
                    x_fp = self.prev_quantized  # dequantized activation for fallback paths
        ...
        if (
            ...
            if use_modiff and self.prev_output is not None:
                # Fused GEMM + Accumulate
                # gemm_w8a8_accum(A, B, scale_a, scale_b, cache) -> C
                prev_out_2d = self.prev_output.view(-1, weight_int8.shape[1])
                out_2d = gemm_w8a8_accum(
                    x_2d, weight_int8, act_scale, self.weight_scale, prev_out_2d
                )

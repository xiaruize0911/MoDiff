...
#pragma unroll
    for (int j = 0; j < NJ; ++j) {
      ...
      if (row0 < M) {
        if (c0) *(__half2*)&C[(size_t)row0 * n_out + col0] = __halves2half2(__float2half(accv[0] * s00), __float2half(accv[1] * s01));
        if (c1) *(__half2*)&C[(size_t)row0 * n_out + col1] = __halves2half2(__float2half(accv[4] * s10), __float2half(accv[5] * s11));
      }
      if (row1 < M) {
        if (c0) *(__half2*)&C[(size_t)row1 * n_out + col0] = __halves2half2(__float2half(accv[2] * s00), __float2half(accv[3] * s01));
        if (c1) *(__half2*)&C[(size_t)row1 * n_out + col1] = __halves2half2(__float2half(accv[6] * s10), __float2half(accv[7] * s11));
      }
    }
  }
}
...
// A [M,K] int8, B [N,K] int8, w_scale [N], a_scale, inv_out_scale [N] (127/absmax_col) -> C [M,N] int8.
// Output-fused int8 Linear: halves the output write vs gemm_w8a8_awq; dequant with 1/inv_out_scale.
...
// scalar -> C [M,N] fp16. Production int8 Linear entry point.
torch::Tensor gemm_w8a8_awq(torch::Tensor A, torch::Tensor B, torch::Tensor w_scale, double a_scale) {
  ...
  gemm_w8a8_kernel_awq<<<grid, GWQ_NUM_WARPS * 32, 0, stream>>>(
      A.data_ptr<int8_t>(), B.data_ptr<int8_t>(), w_scale.contiguous().data_ptr<float>(),
      (float)a_scale, reinterpret_cast<__half*>(C.data_ptr<at::Half>()), M, N, K, N);
  return C;
}

// Unpadded-output variant: B/w_scale are padded to N%128==0, but the result is written directly as
// [M, n_out] (n_out = real out_features, must be even), skipping the padded columns. This removes the
// downstream `out[:, :n_out].contiguous()` slice-copy on padded qkv/proj GEMMs.
torch::Tensor gemm_w8a8_awq_nout(torch::Tensor A, torch::Tensor B, torch::Tensor w_scale, double a_scale, int64_t n_out) {
  ...
  gemm_w8a8_kernel_awq<<<grid, GWQ_NUM_WARPS * 32, 0, stream>>>(
      A.data_ptr<int8_t>(), B.data_ptr<int8_t>(), w_scale.contiguous().data_ptr<float>(),
      (float)a_scale, reinterpret_cast<__half*>(C.data_ptr<at::Half>()), M, N, K, (int)n_out);
  return C;
}
...
// logical (N%128==0, K%128==0 -- pad at call site) -> C [M,N] fp16. Production int4 Linear entry point.
torch::Tensor gemm_w4a4_awq(torch::Tensor A, torch::Tensor B, torch::Tensor w_scale, double a_scale, int64_t K) {
  ...
  gemm_w4a4_kernel_awq<<<grid, GWQ_NUM_WARPS * 32, 0, stream>>>(
      A.data_ptr<int8_t>(), B.data_ptr<int8_t>(), w_scale.contiguous().data_ptr<float>(),
      (float)a_scale, reinterpret_cast<__half*>(C.data_ptr<at::Half>()), M, N, Kb, N);
  return C;
}

// Unpadded-output int4 variant (see gemm_w8a8_awq_nout): writes [M, n_out] directly, skipping the
// padded columns -> removes the downstream slice+.contiguous() copy on padded qkv/proj int4 GEMMs.
torch::Tensor gemm_w4a4_awq_nout(torch::Tensor A, torch::Tensor B, torch::Tensor w_scale, double a_scale, int64_t K, int64_t n_out) {
  ...
  gemm_w4a4_kernel_awq<<<grid, GWQ_NUM_WARPS * 32, 0, stream>>>(
      A.data_ptr<int8_t>(), B.data_ptr<int8_t>(), w_scale.contiguous().data_ptr<float>(),
      (float)a_scale, reinterpret_cast<__half*>(C.data_ptr<at::Half>()), M, N, Kb, (int)n_out);
  return C;
}

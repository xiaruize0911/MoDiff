...
// ---- ENTRYPOINT (materialized path, step 1/3). Kernel: bmm_qk_s8_kernel. ----
//   Op:       Attention W8A8 — QKᵀ
//   Inputs:   Q int8 [BH,T,hd_pad], K int8 [BH,T,hd_pad], sq f32 [BH,T] (per-token),
//             sk f32 [BH,T] (per-token), scale double (= 1/sqrt(hd))
//   Outputs:  S fp16 [BH,T,T]  (dequantized + scaled logits, ready for softmax)
//   Computes: S[i,j] = (Σ_d Q[i,d]·K[j,d]) · sq[i] · sk[j] · scale
//   Fuses:    per-token dequant (sq[i]·sk[j]) + softmax scale folded into the QKᵀ epilogue;
//             int32 acc → fp16 scaled logits directly (no fp32 raw-score dump, halves the S write)
//   Constraints: hd_pad%32==0, T%64==0 (T%AQ_BN==0, AQ_BN=64)
//   vs fp16:  int8 materialized (attn_qk_int8+softmax_requant+attn_av_int8) ~0.93x / int4 ~1.01x vs
//             fp16 MATH (weighted over churches shapes); loses at small T. The [BH,T,T] scores
//             round-trip HBM 3x (QKᵀ writes, softmax reads+writes int8 P, AV reads) — the memory-
//             bound bottleneck the flash kernel eliminates.
...
// ---- ENTRYPOINT (materialized path, step 3/3). Kernel: bmm_av_s8_kernel. ----
//   Op:       Attention W8A8 — AV
//   Inputs:   P int8 [BH,T,T] (softmax probs in [0,127]), Vt int8 [BH,hd_pad,T] (V transposed to
//             channel-major = AV B operand), sp f32 [BH,T] (per-row softmax scale), sv f32
//             [BH,hd_pad] (per-channel V scale)
//   Outputs:  O fp16 [BH,T,hd_pad]
//   Computes: O[i,d] = (Σ_j P[i,j]·Vt[d,j]) · sp[i] · sv[d]
//   Fuses:    per-row sp[i] · per-channel sv[d] dequant folded into the AV epilogue; sp carries the
//             deferred 1/(127·Σexp) softmax normalizer from attn_softmax_requant
//   Constraints: T%32==0, hd_pad%64==0 (hd_pad%AQ_BN==0)
//   vs fp16:  int8 materialized (attn_qk_int8+softmax_requant+attn_av_int8) ~0.93x / int4 ~1.01x vs
//             fp16 MATH (weighted over churches shapes); loses at small T. The [BH,T,T] scores
//             round-trip HBM 3x (QKᵀ writes, softmax reads+writes int8 P, AV reads) — the memory-
//             bound bottleneck the flash kernel eliminates.
...
// ---- ENTRYPOINT (materialized path, step 2/3). Kernel: attn_softmax_requant_kernel. ----
//   Op:       Attention W8A8 — softmax+requant
//   Inputs:   S fp16 [BH,T,T]  (pre-scaled logits; QKᵀ epilogue already applied sq·sk·scale)
//   Outputs:  P int8 [BH,T,T] in [0,127], sp f32 [BH,T]
//   Computes: per row i: m=max_j S[i,j]; P[i,j]=round(exp(S[i,j]−m)·127); sp[i]=1/(127·Σ_j exp)
//   Fuses:    2 passes over the T×T row (down from 3): (1) row max, (2) exp + Σexp accumulate AND
//             int8 P write in the same pass; the 1/(127·Σexp) normalizer is deferred into sp and
//             applied later in the AV epilogue (P needs only the max, not the sum)
//   Constraints: T%8==0 (128-bit vectorized rows)
...
// ---- ENTRYPOINT (materialized int4 path, step 1/3). Kernel: bmm_qk_s4_kernel. ----
//   Op:       Attention W4A4 — QKᵀ
//   Inputs:   Q packed-int4 [BH,T,hd_pad/2] (2 nibbles/byte, dtype kChar), K packed-int4
//             [BH,T,hd_pad/2], hd_pad int64 (logical head dim), sq f32 [BH,T], sk f32 [BH,T],
//             scale double (= 1/sqrt(hd))
//   Outputs:  S fp16 [BH,T,T]  (dequantized + scaled logits)
//   Computes: S[i,j] = (Σ_d Q[i,d]·K[j,d] over hd_pad int4 lanes) · sq[i] · sk[j] · scale
//   Fuses:    per-token dequant + softmax scale folded into the QKᵀ epilogue (mma.m16n8k64.s4)
//   Constraints: hd_pad%64==0, T%64==0
//   vs fp16:  int8 materialized ~0.93x / int4 ~1.01x vs fp16 MATH (weighted over churches shapes);
//             loses at small T. The [BH,T,T] scores round-trip HBM 3x (QKᵀ writes, softmax
//             reads+writes int4 P, AV reads) — the memory-bound bottleneck the flash kernel removes.
...
// ---- ENTRYPOINT (materialized int4 path, step 2/3). Kernel: attn_softmax_requant4_kernel. ----
//   Op:       Attention W4A4 — softmax+requant
//   Inputs:   S fp16 [BH,T,T]  (pre-scaled logits)
//   Outputs:  P packed-int4 [BH,T,T/2] (dtype kChar, 2 nibbles/byte) in [0,7], sp f32 [BH,T]
//   Computes: per row i: m=max_j S[i,j]; P[i,j]=round(exp(S[i,j]−m)·7) ∈ [0,7]; sp[i]=1/(7·Σ_j exp)
//   Fuses:    2 passes; pass 2 fuses exp + Σexp + int4 requant + pack (8 int4 → one 32-bit store,
//             matching the AV unpack); 1/(7·Σexp) normalizer deferred into sp (applied in AV)
//   Constraints: T%8==0 (128-bit vectorized rows)
...
// ---- ENTRYPOINT (materialized int4 path, step 3/3). Kernel: bmm_av_s4_kernel. ----
//   Op:       Attention W4A4 — AV
//   Inputs:   P packed-int4 [BH,T,T/2] (softmax probs in [0,7]), Vt packed-int4 [BH,hd_pad,T/2]
//             (V transposed to channel-major), sp f32 [BH,T] (per-row scale), sv f32 [BH,hd_pad]
//             (per-channel V scale), T int64 (logical seq len)
//   Outputs:  O fp16 [BH,T,hd_pad]
//   Computes: O[i,d] = (Σ_j P[i,j]·Vt[d,j] over T int4 lanes) · sp[i] · sv[d]
//   Fuses:    per-row sp[i] · per-channel sv[d] dequant folded into the AV epilogue (mma.m16n8k64.s4);
//             sp carries the deferred 1/(7·Σexp) softmax normalizer
//   Constraints: T%64==0, hd_pad%64==0
//   vs fp16:  int8 materialized ~0.93x / int4 ~1.01x vs fp16 MATH (weighted over churches shapes);
//             loses at small T. The [BH,T,T] scores round-trip HBM 3x — the memory-bound bottleneck
//             the flash kernel eliminates.
...
// ---- ENTRYPOINT (static score path). Kernel: attn_softmax_requant_static_kernel. ----
//   Op:       Attention W8A8 — softmax+requant (static)
//   Inputs:   S fp16 [BH,T,T] (pre-scaled logits), c double (calibrated per-layer max upper bound)
//   Outputs:  P int8 [BH,T,T] in [0,127], sp f32 [BH,T]
//   Computes: P[i,j]=round(exp(S[i,j]−c)·127); sp[i]=1/(127·Σ_j exp)  (no per-row max)
//   Fuses:    calibrated c replaces the row-max pass → SINGLE read of the T×T score matrix (vs 2 for
//             the dynamic kernel); exp+Σexp+int8 write in that one pass; normalizer deferred to sp
//   Constraints: T%8==0; requires calibrated c ≥ actual row max (else coarser int8 resolution)
...
// ---- ENTRYPOINT (static int4 score path). Kernel: attn_softmax_requant4_static_kernel. ----
//   Op:       Attention W4A4 — softmax+requant (static)
//   Inputs:   S fp16 [BH,T,T] (pre-scaled logits), c double (calibrated per-layer max upper bound)
//   Outputs:  P packed-int4 [BH,T,T/2] in [0,7], sp f32 [BH,T]
//   Computes: P[i,j]=round(exp(S[i,j]−c)·7) ∈ [0,7], packed 2/byte; sp[i]=1/(7·Σ_j exp)
//   Fuses:    static c → single T×T read; exp+Σexp+int4 requant+pack in that pass; normalizer in sp
//   Constraints: T%8==0; requires calibrated c ≥ actual row max
...
// ---- ENTRYPOINT (int8-qkv-linear → attention quantize fusion, prototype). Kernels:
//      aq_qtok_from_i8_kernel (Q/K), aq_vscale_from_i8_kernel + aq_vquant_trans_from_i8_kernel (V). ----
//   Op:       Attention W8A8 — Q/K/V quantize (from int8 qkv-linear)
//   Inputs:   qkv_i8 int8 [b*T, 3C] (fused int8 qkv-linear output, e.g. fused_gn_qkv_int8; columns
//             ordered (nh,3,hd), so col (h,which,d)=(h*3+which)*hd+d), oscale f32 [3C] (per-column
//             output scale; true value = i8/oscale), nh int64, T int64, hp_qk int64, hp_av int64
//   Outputs:  qi,ki int8 [BH,T,hp_qk], vt int8 [BH,hp_av,T] (channel-major), sq,sk f32 [BH,T]
//             (per-token), sv f32 [BH,hp_av] (per-channel) — same layout as quantize_attn_qkv
//   Computes: dequant on the fly (i8 · 1/oscale), re-quantize to attention granularity (per-token
//             Q/K, per-channel V transposed to channel-major)
//   Fuses:    skips the fp16 round-trip between the int8 qkv-linear and the attention quantize
//   Constraints: int8 (W8A8) only; 3C ordered (nh,3,hd)
...
// ---- ENTRYPOINT (int8-score static softmax). Kernel: attn_softmax_requant_s8_kernel. ----
//   Op:       Attention W8A8 — softmax+requant (int8-score, static)
//   Inputs:   S int8 [BH,T,T] (int8 scores; dequant per-tensor sS), sS double (score scale =
//             calib_absmax/127), c double (calibrated static max)
//   Outputs:  P int8 [BH,T,T] in [0,127], sp f32 [BH,T]
//   Computes: logit=S_i8·sS; P[i,j]=round(exp(logit−c)·127); sp[i]=1/(127·Σ_j exp)  (single pass)
//   Fuses:    int8 score storage halves the dominant T×T read vs fp16 S; static c → single pass;
//             normalizer deferred to sp. Same output as attn_softmax_requant_static.
//   Constraints: T%8==0
...
// ---- ENTRYPOINT (int8-score dynamic softmax). Kernel: attn_softmax_requant_s8_dyn_kernel. ----
//   Op:       Attention W8A8 — softmax+requant (int8-score, dynamic)
//   Inputs:   S int8 [BH,T,T] (int8 scores; dequant per-tensor sS), sS double (per-tensor score scale)
//   Outputs:  P int8 [BH,T,T] in [0,127], sp f32 [BH,T]
//   Computes: logit=S_i8·sS; per-row m=max_j logit; P[i,j]=round(exp(logit−m)·127); sp[i]=1/(127·Σexp)
//   Fuses:    int8 S halves the T×T read; DYNAMIC per-row max keeps quality (no static-c loss).
//             Paired with attn_qk_int8_s8out (int8 S write) this halves BOTH T×T passes.
//   Constraints: T%8==0
...
// ---- ENTRYPOINT (int8-score QKᵀ). Kernel: bmm_qk_s8_i8out_kernel. ----
//   Op:       Attention W8A8 — QKᵀ (int8-score out)
//   Inputs:   Q int8 [BH,T,hd_pad], K int8 [BH,T,hd_pad], sq f32 [BH,T], sk f32 [BH,T], scale
//             double (= 1/sqrt(hd)), sS double (score scale; S_i8 = round(logit/sS))
//   Outputs:  S int8 [BH,T,T] in [-127,127]
//   Computes: logit=(Σ_d Q[i,d]·K[j,d])·sq[i]·sk[j]·scale; S[i,j]=clamp(round(logit/sS))
//   Fuses:    same int8 QKᵀ mainloop as bmm_qk_s8_kernel, but the epilogue writes int8 scores →
//             halves the T×T write vs fp16 S; pairs with attn_softmax_requant_s8(_dyn)
//   Constraints: hd_pad%32==0, T%64==0

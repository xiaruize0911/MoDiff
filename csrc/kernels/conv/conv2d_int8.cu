...
//
//   Op: Conv2d W8A8 -- CUTLASS implicit-GEMM int8 convolution, NHWC (Ampere/Sm80).
//
...
//
// Naming convention (suffixes compose; verified against the functions below):
//   *_fprop              = base CUTLASS implicit-GEMM conv; int32 accumulate, output
//                          scaled only by the scalar activation alpha (`scales`/`inv_scale`)
//                          -- per-channel weight scale NOT applied here.
//   *_dequant_fp16*      = "deep-fuse" epilogue: per-channel weight scale folded into the
//                          CUTLASS epilogue, writes FP16 directly (no FP32 temporary).
//   *_tuned / *_num_tuned_configs = threadblock/warp tile is a template param; the Python
//                          autotuner picks the fastest `config_id` per shape.
//   *_o_hat              = MoDiff delta cache path: o_hat_cache += conv * weight_scale[ch]
//                          (in-place accumulate via a separate scale_accumulate kernel).
//   *_no_ohat[_prealloc] = plain per-channel dequant store (conv * weight_scale[ch]), no
//                          cache; _prealloc writes into a caller-provided output buffer.
//   *_bias               = + per-channel bias, added in the store epilogue.
//   *_bias_residual      = + per-channel bias + per-element skip residual (ResBlock add).
//   *_relu_requant_int8  = dequant(+bias) + optional ReLU, requantized to INT8 output
//                          (keeps the chain int8->int8, no fp16 round-trip); uses an FP32 temp.
//   *_deepfuse_*         = same fusions, but the dequant runs in the CUTLASS epilogue (FP16
//                          scratch, no FP32 temp); covers relu_requant_int8 and bias_residual.
//   *_dual               = DUAL output: FP16 block output (x_{N+1}) + requantized INT8 (next
...
//   Inputs:   input int8 [N,C,H,W] channels_last; weight int8 [K,R,S,C]; inv_scale fp32
//             (1-elem device ptr = activation alpha); weight_scales_half fp16 [K] (per-channel
//             weight scale, broadcast as the epilogue "C" source); output fp16 [N,K,H_out,W_out]
//             channels_last, PREallocated; stride/pad/dilation.
//   Outputs:  output (fp16, in place) = int32_accum * inv_scale * weight_scale[ch].
//   Fuses:    dequant (scalar alpha + per-channel weight scale) inside CUTLASS epilogue -- no FP32 temp.
//   Constraints: output/weight_scales_half must be fp16, weight_scales_half.numel()==K; deep-fuse
//             epilogue vectorizes 8 (K % 8 == 0); fixed 128x128x128 tile.
...
// Number of deep-fuse fp16 tile configs the autotuner may pick from.
//   Inputs:   none.   Outputs: int64 count (currently 8; valid config_id range [0,8)).
//   Fuses:    none (metadata for conv2d_int8_dequant_fp16_tuned / *_deepfuse_*).
...
//             weight_scales_half fp16 [K]; output fp16 [N,K,H_out,W_out] cl PREallocated;
//             config_id int64 (0..num_tuned_configs-1); stride/pad/dilation.
//   Outputs:  output (fp16, in place) = int32_accum * inv_scale * weight_scale[ch].
//   Fuses:    same deep-fuse dequant->fp16 as *_dequant_fp16_prealloc, but with tile `config_id`.
//   Constraints: output/weight_scales_half fp16; K % 8 == 0; raises if the tile can't implement the shape.
...
//   Inputs:   input int8 [N,C,H,W] cl; weight int8 [K,R,S,C]; inv_scale fp32 (1-elem alpha);
//             weight_scales fp32 [K]; output fp32 or fp16 [N,K,H_out,W_out] cl PREallocated;
//             stride/pad/dilation.
...
//   Inputs:   input int8; weight int8; inv_scale fp32 (1-elem alpha); weight_scales fp32 [K];
...
//   Inputs:   input int8; weight int8; inv_scale fp32 (1-elem alpha); weight_scales fp32 [K];
//             bias fp32/fp16 [K] or empty; residual fp16 [N,K,H_out,W_out] cl; output fp16
//             (same shape) PREallocated.
...
//   Inputs:   input int8; weight int8; inv_scale fp32 (1-elem alpha); weight_scales fp32 [K];
//             bias fp32/fp16 [K] or empty; requant_scale fp32 (1-elem = NEXT conv's 127/absmax);
//             output int8 [N,K,H_out,W_out] cl PREallocated; apply_relu bool; stride/pad/dilation.
//   Outputs:  output (int8, in place) = round(clamp((conv*ws[ch]+bias[ch])[,relu] * requant_scale, +-127)).
//   Fuses:    per-channel dequant + optional bias + optional ReLU + requantize to INT8 (via an FP32 temp).
//   Constraints: output int8, matching shape; requant_scale is a scalar; bias (if given) numel()==K.
...
//   Inputs:   input int8; weight int8; inv_scale fp32 (1-elem alpha); weight_scales_half fp16 [K]
//             (folded into the CUTLASS epilogue); bias fp32/fp16 [K] or empty; requant_scale fp32
//             (1-elem = NEXT conv's 127/absmax); output int8 [N,K,H_out,W_out] cl PREallocated;
//             apply_relu bool; config_id int64 (<0 = fixed default tile); stride/pad/dilation.
//   Outputs:  output (int8, in place) = round(clamp((deq_fp16+bias[ch])[,relu] * requant_scale, +-127)).
//   Fuses:    deep-fuse dequant (weight scale in epilogue, fp16 scratch, NO fp32 temp) + bias
//             + optional ReLU + requantize to INT8.
//   Constraints: output int8; requant_scale scalar; weight_scales_half fp16; K % 8 == 0; config_id in [-1,8).
...
// Deep-fuse + tunable FP16-output conv with per-channel bias + OPTIONAL skip residual --
// the tuned/no-fp32-temp replacement for conv2d_int8_fprop_no_ohat_prealloc_bias[_residual]
// (the best fused INT8 store path — bias folded into the from_half store, no eager add).
// config_id<0 = fixed default tile. residual empty (numel 0) -> bias-only store.
//   Inputs:   input int8; weight int8; inv_scale fp32 (1-elem alpha); weight_scales_half fp16 [K];
//             bias fp32/fp16 [K] or empty; residual fp16 [N,K,H_out,W_out] cl or empty; output fp16
//             (same shape) PREallocated; config_id int64 (<0 = fixed default tile); stride/pad/dilation.
//   Outputs:  output (fp16, in place) = deq_fp16 + bias[ch] (+ residual[i] if given).
//   Fuses:    deep-fuse dequant (weight scale in epilogue, NO fp32 temp) + optional bias + optional residual.
//   Constraints: output (& residual if given) fp16; weight_scales_half fp16; K % 8 == 0; bias numel()==K or 0.
    ...
    if (has_res) {
        ...
        if (bias_half)
            bias_store_half_from_half_kernel<__half><<<grid_size, block_size, 0, stream>>>(
                deq_ptr, reinterpret_cast<const __half*>(bias.data_ptr<at::Half>()), out_ptr, num_elements, num_channels);
        else if (has_bias)
            bias_store_half_from_half_kernel<float><<<grid_size, block_size, 0, stream>>>(
                deq_ptr, bias.data_ptr<float>(), out_ptr, num_elements, num_channels);
        else
            bias_store_half_from_half_kernel<__half><<<grid_size, block_size, 0, stream>>>(
                deq_ptr, (const __half*)nullptr, out_ptr, num_elements, num_channels);
    }
    return output;
}
...
//   Inputs:   input int8; weight int8; inv_scale fp32 (1-elem alpha); weight_scales_half fp16 [K];
//             bias fp32/fp16 [K] or empty; residual fp16 [N,K,H_out,W_out] cl; requant_scale fp32
//             (1-elem = next block conv1's 127/absmax); out_half fp16 (same shape); out_int8 int8
//             (same shape); apply_relu bool; config_id int64 (<0 = default tile); stride/pad/dilation.
//   Outputs:  DUAL (both in place): out_half (fp16, returned) = (deq+bias[ch]+residual)[,relu];
//             out_int8 = round(clamp(out_half * requant_scale, +-127)).
//   Fuses:    deep-fuse dequant (NO fp32 temp) + optional bias + skip residual + optional ReLU
//             + block-entry requantize to INT8, in one store.
//   Constraints: out_half & residual fp16; out_int8 int8; requant_scale scalar; weight_scales_half
//             fp16; K % 8 == 0.

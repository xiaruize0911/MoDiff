...
// The bias-fused variant (w8a8_gemm_fuse_bias_forward_cuda / dense_kernel0_fuse_bias)
// is present but NOT exported. Requires N%128==0, K%64==0. See the per-function header
// block above w8a8_gemm_forward_cuda for the full I/O contract.
// =============================================================================
                              ...
                              int M, int N, int K)
{
  ...
  // Guard against N not being a multiple of CTA_N: the last n-tile would
  // otherwise read Bias[cta_offset_n + i] past the end of the length-N bias
  // array (an unconditional OOB global read that corrupts memory / faults
  // depending on allocator layout). Padding columns get a 0 bias; they are
  // never stored (the epilogue guards on col_wb < N).
  #pragma unroll
  for (int i = 0; i < CTA_N ; i++)
  {
    Bias_shared[i] = (cta_offset_n + i) < N ? __half2float(Bias[cta_offset_n+i]) : 0.0f;
  }
...
// =============================================================================
//   Op:       Linear W8A8 GEMM (AWQ vendored kernel; exported to Python as `awq_w8a8_gemm`)
//   Inputs:   _in_feats int8 [M, K]  (M=num_in_feats, K=num_in_channels; quantized activation)
//             _kernel   int8 [N, K]  (N=num_out_channels; weight, one row per output channel)
//             _wscales  fp16 [N]     (per-channel weight scale; read pairwise as half2)
//             _ascales  fp16 [M]     (PER-TOKEN activation scale, one per output row)
//             _out_feats fp16 [M, N] PREALLOCATED (written in place)
//   Outputs:  writes into _out_feats fp16 [M, N] (function returns void; output is caller-allocated)
//   Computes: C[m,n] = (A[m,:].B[n,:] int8) * ascales[m] * wscales[n]     (no bias)
//   Fuses:    fp16 dequant epilogue (per-token ascale * per-channel wscale). NOTE: unlike
//             gemm_w8a8_awq's per-tensor scalar a_scale, this takes a per-token ascales[M] vector and
//             an fp16 (not f32) wscale. The bias-fused sibling (dense_kernel0_fuse_bias /
//             w8a8_gemm_fuse_bias_forward_cuda) lives in this file but is NOT bound in pybind.cpp.
//   Constraints: N%128==0, K%64==0 (per pybind note). CTA config selected by M: M>128 uses
//             CTA_M/N=128, STAGES=3; otherwise CTA_M/N=64, STAGES=6.
